<?php
// =================================================================
//  Parceiro de Programação - Relatório de KM Consolidado (v21.0)
//  - VISÃO DIÁRIA (ABA 1): Mantida V15.0.
//  - VISÃO CONSOLIDADA (ABA 2): Atualizada para visual V2.8 (Cards Coloridos e Detalhados).
//  - MATEMÁTICA: Migração para relatorios_viagens_precisao (LENGTH_KM).
//  - REGRA DESCRIPTION: Revenue = Produtivo | DeadHead/Fuori_linea = Ocioso.
// =================================================================
...
// B. MTRAN (Geral e Ajuste) - MATEMÁTICA ATUALIZADA
if (empty($veiculo_filtro)) {
    // Nova lógica de ociosidade baseada em DESCRIPTION e LENGTH_KM
    $case_prod = "CASE WHEN (description = 'Revenue') THEN CAST(REPLACE(length_km, ',', '.') AS DECIMAL(15,3)) ELSE 0 END";
    $case_ocio = "CASE WHEN (description IN ('DeadHead', 'Fuori_linea')) THEN CAST(REPLACE(length_km, ',', '.') AS DECIMAL(15,3)) ELSE 0 END";

    // MTRAN Padrão (Agora da relatorios_viagens_precisao)
    $sql_mtran = "SELECT data_viagem, SUM($case_prod)/1000.0 as p, SUM($case_ocio)/1000.0 as o FROM relatorios_viagens_precisao WHERE data_viagem $where_periodo GROUP BY data_viagem";
    $res = $conexao->query($sql_mtran);
    ...
    // MTRAN Ajuste (Mantido conforme original, já que a tabela de ajuste não mudou)
    $case_prod_aj = "CASE WHEN (TRIP_ID = 0) OR (ROUTE_ID IN ('2', '002') AND data_viagem > '2025-12-21') THEN 0 ELSE CAST(DISTANCE AS DECIMAL(10,2)) END";
    $case_ocio_aj = "CASE WHEN (TRIP_ID = 0) OR (ROUTE_ID IN ('2', '002') AND data_viagem > '2025-12-21') THEN CAST(DISTANCE AS DECIMAL(10,2)) ELSE 0 END";

    $res_aj = $conexao->query("SELECT data_viagem, SUM($case_prod_aj)/1000.0 as p, SUM($case_ocio_aj)/1000.0 as o FROM relatorios_viagens_ajuste WHERE data_viagem $where_periodo GROUP BY data_viagem");
    if($res_aj) while($r = $res_aj->fetch_assoc()) {
        $d = $r['data_viagem']; $p = (float)$r['p']; $o = (float)$r['o']; $t = get_day_type($d);
        init_registro_dia($dados_para_js, $d);
...
<body class="p-4">
                ...
                <table class="main-table w-full" id="tabela-resumo">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr onclick="toggleDetails('<?= $rowId ?>')" class="border-b main-row">
                                ...
                                <td><?= number_format($cols['prog_planilha'], 3, ',', '.') ?></td>
                                <td class="bg-indigo-50 font-semibold text-indigo-700"><?= number_format($cols['prog_mtran_prod'], 3, ',', '.') ?></td>
                                <td class="bg-indigo-50 text-indigo-600"><?= number_format($cols['prog_mtran_ocio'], 3, ',', '.') ?></td>
                                <td class="bg-blue-100 border-l-2 border-blue-200 font-bold text-blue-800"><?= number_format($cols['ajuste_mtran_prod'], 3, ',', '.') ?></td>
                                <td class="bg-blue-100 border-r-2 border-blue-200 text-blue-700"><?= number_format($cols['ajuste_mtran_ocio'], 3, ',', '.') ?></td>
                                <td class="bg-green-50 text-green-700"><?= number_format($cols['prog_004_prod'], 3, ',', '.') ?></td>
                                <td class="bg-green-50 text-green-600"><?= number_format($cols['prog_004_ocio'], 3, ',', '.') ?></td>
                                <td class="bg-yellow-50 font-bold text-yellow-900 border-l-2 border-yellow-200"><?= number_format($cols['real_roleta'], 3, ',', '.') ?></td>
                                <td class="bg-blue-50 text-blue-900"><?= number_format($cols['real_life'], 3, ',', '.') ?></td>
                                <td class="bg-pink-50 text-pink-900"><?= number_format($cols['real_142'], 3, ',', '.') ?></td>
                                <td class="bg-cyan-50 text-cyan-900 font-bold"><?= number_format($cols['real_noxxon'], 3, ',', '.') ?></td>
                                <td class="bg-green-100 font-bold text-green-900"><?= number_format($cols['real_004_prod'], 3, ',', '.') ?></td>
                            </tr>
                    ...
                    <tfoot>
                        <tr class="bg-slate-800 text-white font-bold text-center text-base">
                            ...
                            <td><?= number_format($totais_tabela['prog_planilha'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['prog_mtran_prod'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['prog_mtran_ocio'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['ajuste_mtran_prod'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['ajuste_mtran_ocio'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['prog_004_prod'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['prog_004_ocio'], 3, ',', '.') ?></td>
                            <td class="text-yellow-300"><?= number_format($totais_tabela['real_roleta'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['real_life'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['real_142'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['real_noxxon'], 3, ',', '.') ?></td>
                            <td><?= number_format($totais_tabela['real_004_prod'], 3, ',', '.') ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <section class="mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="summary-card-prog p-5 rounded-lg shadow-lg flex flex-col justify-between summary-card">
                    <div>
                        ...
                        <p class="text-4xl font-bold"><?= number_format($dados_consolidados_stats['planilha']['total'], 3, ',', '.') ?> km</p>
                        ...
                        <?php 
                            ...
                            $ano_ref_meta = date('Y', strtotime($data_inicio));
                        ?>
                        <p class="text-sm opacity-80 mt-1">
                            Ref. Mês (<?= $mes_nome ?> de <?= $ano_ref_meta ?>): <?= number_format($dados_consolidados_stats['planilha']['meta_mes_ref'], 0, ',', '.') ?> km
                        </p>
                    </div>
                ...
                <div class="summary-card-prog-mtran p-5 rounded-lg shadow-lg flex flex-col justify-between summary-card">
                    <div>
                        ...
                        <p class="text-4xl font-bold"><?= number_format($dados_consolidados_stats['mtran']['total'], 3, ',', '.') ?> km</p>
                    </div>
                    <div class="mt-2 text-sm space-y-1">
                        <p>Produtivo: <span class="font-semibold"><?= number_format($dados_consolidados_stats['mtran']['prod'], 3, ',', '.') ?> km</span></p>
                        <p>Ocioso: <span class="font-semibold"><?= number_format($dados_consolidados_stats['mtran']['ocio'], 3, ',', '.') ?> km</span></p>
                    </div>
                    <ul class="mt-2 text-xs opacity-90">
                        <li><span>Produtivo Úteis:</span> <span><?= number_format($dados_consolidados_stats['mtran']['prod_u'], 3, ',', '.') ?> km</span></li>
                        <li><span>Produtivo Sábados:</span> <span><?= number_format($dados_consolidados_stats['mtran']['prod_s'], 3, ',', '.') ?> km</span></li>
                        <li><span>Produtivo Domingos:</span> <span><?= number_format($dados_consolidados_stats['mtran']['prod_d'], 3, ',', '.') ?> km</span></li>
                        <li class="mt-1 pt-1 border-t border-white/10"><span>Ocioso Úteis:</span> <span><?= number_format($dados_consolidados_stats['mtran']['ocio_u'], 3, ',', '.') ?> km</span></li>
                        <li><span>Ocioso Sábados:</span> <span><?= number_format($dados_consolidados_stats['mtran']['ocio_s'], 3, ',', '.') ?> km</span></li>
                        <li><span>Ocioso Domingos:</span> <span><?= number_format($dados_consolidados_stats['mtran']['ocio_d'], 3, ',', '.') ?> km</span></li>
                    </ul>
                </div>
                ...
                <div class="summary-card-prog-004 p-5 rounded-lg shadow-lg flex flex-col justify-between summary-card">
                    <div>
                        ...
                        <p class="text-4xl font-bold"><?= number_format($dados_consolidados_stats['clever004_p']['total'], 3, ',', '.') ?> km</p>
                    </div>
                    <div class="mt-2 text-sm space-y-1">
                        <p>Produtivo: <span class="font-semibold"><?= number_format($dados_consolidados_stats['clever004_p']['prod'], 3, ',', '.') ?> km</span></p>
                        <p>Ocioso: <span class="font-semibold"><?= number_format($dados_consolidados_stats['clever004_p']['ocio'], 3, ',', '.') ?> km</span></p>
                    </div>
                    <ul class="mt-2 text-xs opacity-90">
                        <li><span>Produtivo Úteis:</span> <span><?= number_format($dados_consolidados_stats['clever004_p']['prod_u'], 3, ',', '.') ?> km</span></li>
                        <li><span>Produtivo Sábados:</span> <span><?= number_format($dados_consolidados_stats['clever004_p']['prod_s'], 3, ',', '.') ?> km</span></li>
                        <li><span>Produtivo Domingos:</span> <span><?= number_format($dados_consolidados_stats['clever004_p']['prod_d'], 3, ',', '.') ?> km</span></li>
                    </ul>
                </div>

                <div class="card bg-white p-5 rounded-lg shadow-lg flex flex-col justify-between summary-card" style="color:#333">
                    <div>
                        <h3 class="font-semibold text-gray-700 mb-1">KM Realizado (Roleta)</h3>
                        <p class="text-4xl font-bold text-yellow-600"><?= number_format($dados_consolidados_stats['roleta']['total'], 3, ',', '.') ?> km</p>
                    </div>
                    <div class="mt-2 text-sm text-gray-600 space-y-1">
                        <p>Dias Úteis: <span class="font-semibold"><?= number_format($dados_consolidados_stats['roleta']['u'], 3, ',', '.') ?> km</span></p>
                        <p>Sábados: <span class="font-semibold"><?= number_format($dados_consolidados_stats['roleta']['s'], 3, ',', '.') ?> km</span></p>
                        <p>Domingos: <span class="font-semibold"><?= number_format($dados_consolidados_stats['roleta']['d'], 3, ',', '.') ?> km</span></p>
                    </div>
                </div>
            </section>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                <div class="bg-white p-6 rounded-lg shadow lg:col-span-1">
                    ...
                    <ul class="space-y-3 text-sm">
                        ...
                        foreach($itens_comparacao as $lbl => $val): 
                            $diff_val = $val - $dados_consolidados_stats['roleta']['total'];
                            $perc_val = calcular_diff_percentual($val, $dados_consolidados_stats['roleta']['total']);
                        ?>
                        <li class="flex justify-between items-center">
                            <span class="font-medium text-gray-600"><?= $lbl ?></span>
                            <div class="text-right">
                                <span class="<?= get_cor_diff_abs($diff_val) ?> block"><?= number_format($diff_val, 3, ',', '.') ?> km</span>
                                <span class="<?= get_cor_diff_perc($perc_val) ?> text-xs font-bold">(<?= number_format($perc_val, 3, ',', '.') ?>%)</span>
                            </div>
                        </li>
                        <?php endforeach; ?>
                    </ul>
                ...
                <div class="bg-white p-6 rounded-lg shadow lg:col-span-2 card-km-top">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <?php if (!empty($top_veiculos)): foreach ($top_veiculos as $fonte => $lista): ?>
                            <div>
                                <h4 class="font-bold mb-2 text-xs uppercase tracking-wide"><?= $fonte ?></h4>
                                <ul><?php foreach($lista as $v): ?><li><span><?= $v['vehicle'] ?></span> <strong><?= number_format($v['km'], 3, ',', '.') ?></strong></li><?php endforeach; ?></ul>
                            </div>
                                    ...
                                    <table class="w-full min-w-full divide-y divide-gray-200 text-xs text-center">
                                        ...
                                        <tbody class="divide-y divide-gray-200">
                                            ...
                                            <tr>
                                                ...
                                                <td class="bg-yellow-50 p-2"><?= $rol!==null?number_format($rol,3,',','.'):'-' ?></td>
                                                <td class="bg-blue-50 p-2"><?= $nox!==null?number_format($nox,3,',','.'):'-' ?></td>
                                                <td class="bg-pink-50 p-2"><?= $c142!==null?number_format($c142,3,',','.'):'-' ?></td>
                                                <td class="bg-green-50 p-2"><?= $c004r!==null?number_format($c004r,3,',','.'):'-' ?></td>
                                                <td class="bg-green-50 p-2"><?= $c004p!==null?number_format($c004p,3,',','.'):'-' ?></td>
                                                <td class="bg-indigo-50 p-2"><?= $life!==null?number_format($life,3,',','.'):'-' ?></td>
    ...
    <script>
        ...
        function numberToCsv(n){return parseFloat(n||0).toFixed(3).replace('.',',');}
